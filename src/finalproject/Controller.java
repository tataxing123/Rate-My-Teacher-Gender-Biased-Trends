...
//		}
		...
		ArrayList<String> sortedKeys = new ArrayList<>(data.getKeySet());
		Collections.sort(sortedKeys, Comparator.comparingInt(Integer::parseInt));

		// Add data to the series based on sorted keys
...
//		}
		...
		// Create a sorted list of keys for data1 and data2
		ArrayList<String> sortedKeys = new ArrayList<>(data1.getKeySet());
		Collections.sort(sortedKeys, Comparator.comparingInt(Integer::parseInt));

		// Add data to the series for data1 (M) and data2 (F) based on sorted keys
		for (String key : sortedKeys) {
			series1.getData().add(new XYChart.Data<>(key, data1.get(key)));
			series2.getData().add(new XYChart.Data<>(key, data2.get(key)));
		}
